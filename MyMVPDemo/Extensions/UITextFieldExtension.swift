...
//        self.addTarget(self, action: #selector(textFieldDidChange(_:)), for: .editingChanged)
        ...
        } else if self.keyboardType == .numberPad || self.keyboardType == .phonePad {
    ...
    func validateNumber(str: String) -> String {
        ...
        for (i, s) in string.enumerated() {
            if s == "٠" {
                string.remove(at: string.index(string.startIndex, offsetBy: i))
                string.append("0")
            } else if s == "١" {
                string.remove(at: string.index(string.startIndex, offsetBy: i))
                string.append("1")
    ...
    func isvalidatePhoneNumber() -> Bool {
        ...
        let mailWithoutSpaces = self.text!.trimmingCharacters(in: .whitespacesAndNewlines)
        let emailRegEx = "^[0-9+]{0,1}+[0-9]{5,16}$"
        let emailTest = NSPredicate(format:"SELF MATCHES %@", emailRegEx)
        return emailTest.evaluate(with: mailWithoutSpaces)
        }
}
